        ...
        <div className="card overflow-hidden hover:border-zinc-700 transition-all active:scale-98 group">
          ...
          >
            ...
            {product.featured && (
              <div className="absolute top-3 right-3">
                <span className="text-[11px] bg-gold-500/20 border border-gold-500/30 text-gold-400 px-2 py-0.5 rounded-full">
                  {inUseLabel}
                </span>
              </div>
            )}
          ...
          <div className="p-4">
            <div className="flex items-start justify-between gap-2 mb-1">
              ...
              <ExternalLink size={13} className="text-zinc-400 flex-shrink-0 mt-0.5" />
            </div>
            <p className="text-xs text-zinc-300 mb-2">{product.tagline}</p>
            <p className="text-xs text-zinc-400 leading-relaxed line-clamp-2 mb-3">{product.description}</p>
            <div className="flex items-center justify-between">
              <span className="text-sm font-medium text-gold-400">{product.price}</span>
              <div className="flex gap-1">
                {product.features.slice(0, 2).map((f) => (
                  <span key={f} className="text-[11px] bg-zinc-800 text-zinc-300 px-2 py-0.5 rounded-full">{f}</span>
                ))}
              </div>
            </div>
          </div>
    ...
    <div className="page-container">
      <div className="flex items-center gap-3 mb-2">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-300">{t('products.subtitle')}</p>
        </div>
      </div>
      ...
      >
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-zinc-300 leading-relaxed">{t('products.brandSub')}</p>
        </div>
      ...
      <div className="mt-6 card p-5 text-center border-gold-500/10 bg-gradient-to-br from-amber-950/20 to-zinc-900">
        ...
        <p className="text-xs text-zinc-300 mb-4">{t('products.ctaSub')}</p>
